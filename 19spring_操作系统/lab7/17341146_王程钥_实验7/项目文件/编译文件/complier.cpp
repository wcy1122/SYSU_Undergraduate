...
int size=8;
char sz[]="8";
char filename[][10]={"stone1","stone2","stone3","stone4","shell1","shell2","tf","tf2"};
char type[][10]={"com","com","com","com","bat","bat","com","com"};
char sector[][10]={"37","38","39","40","41","42","43","45"};
char sec_size[][10]={"1","1","1","1","1","1","2","2"};
...
int main()
{
  ...
  { 
    ...
    s[tot++]=0;
    len=strlen(sec_size[i]);
    for(int j=0;j<len;j++)s[tot++]=sec_size[i][j];
    s[tot++]=0;
  }
  ...
  while(fread(&ch,sizeof(char),1,fp))s[tot++]=ch;
  fclose(fp);
  if(tot>44*512)assert(0);
  tot=44*512;
  
  fp=fopen("tf2.com","rb");
  while(fread(&ch,sizeof(char),1,fp))s[tot++]=ch;
  fclose(fp);
  if(tot>46*512)assert(0);
  tot=46*512;
